...
///////////////////////angle parameters//////////////////////////////
float angle_X; // calculate the inclined angle varia`                                                                     `ble of X-axis by accelerometer
float angle_Y; //calculate the inclined angle variable of Y-axis by accelerometer 
float angle0 = -1; //mechanical balance angle (ideally 0 degrees) 
...
//////////////////////PID parameter///////////////////////////////
double kp = 75, ki = 170, kd = 1.5;                   //angle loop parameter
double kp_speed = 2.56, ki_speed = 0.172, kd_speed = 0;   // speed loop parameter
...
void loop() 
{
  ...
  // Serial.println(Turn_pwm);
  if(donutmode){
    if(accumulatedAngle>180){
    if(clock){clock=false;}else{clock=true;}
    if(clock){
      Serial.print(t);
      left =1;
      right =0;
  ...
  //Serial.println("F");
  {
    ...
    Serial.println(val);
    //donutmode =0;
    switch(val)             //switch statementbv
    {
      case 'F': front=200; break; //if val equals F，front=250，car will move forward 
      case 'B': back=-200; break;       //go back
      ...
      case 'N':buzzer(); break;
      case 'T':{ reset=1;i=0;}; break;
      case 'M':{if(donutmode){donutmode = false;}else{donutmode=true;}} break;
    }
  }
...
void PD()
{
  float error = angle+angle0;
  integral +=error*dt;

  PD_pwm = kp * (error) + kd * angle_speed + ki * integral; //PD angle loop control
}
...
void fullrotation(){
  int16_t ggx, ggy, ggz;
  mpu6050.getRotation(&ggx, &ggy, &ggz);

  // Convert raw data to degrees per second
  float gyroX = ggx / 131.0;
  float gyroY = ggy / 131.0;
  float gyroZ = ggz / 131.0;

  // Integrate angular velocity to get angle change
  float deltaX = gyroX * dt;
  float deltaY = gyroY * dt;
  float deltaZ = gyroZ * dt;

  // Accumulate the angles
  accumulatedAngle += deltaZ; // Change this line depending on the axis you want to track

  // Detect full rotation
  if (accumulatedAngle >= 360.0) {
    Serial.println("Full rotation detected!");
